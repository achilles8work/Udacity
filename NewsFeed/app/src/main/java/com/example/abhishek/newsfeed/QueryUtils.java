...
public class QueryUtils {
     ...
     * directly from the class name QueryUtils (and aan object instance of QueryUtils is not needed).
     */
    private QueryUtils() {
    ...
    /**
     * Query the Feed dataset and return a list of {@link Feed} object.
     */
    public static List<Feed> fetchFeedData(String requestUrl) {
        ...
        // Extract relevant feilds from the JSON response and create a list of {@link Feed}s
    ...
    private static List<Feed> extractFeatureFromJson(String feedJson) {
        ...
        // Create an empty ArrayList that we can start adding feeds to
        List<Feed> feeds = new ArrayList<>();

        // Try to parse the JSON response string. If there's problem with the way the JSON
        ...
        try {
            ...
            // which represents a list a feeds.
            JSONArray feedArray = feedObject.getJSONArray("results");

            // For each feed in the FeedArray, create an {@link Feed} object
            for (int i = 0;i < feedArray.length();i++) {

                // Get a single feed at position i within the list of feeds
                JSONObject currentFeed = feedArray.getJSONObject(i);

                String title = currentFeed.getString("sectionName");

                String webTitle = currentFeed.getString("webTitle");

                String url = currentFeed.getString("webUrl");

                Feed feed = new Feed(title, webTitle, url);
